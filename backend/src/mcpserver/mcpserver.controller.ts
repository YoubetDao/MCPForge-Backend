...
export class McpServerController {
  ...
  @Get(":name")
  getMcpServerByName(@Param("name") name: string, @CurrentUser() user: SessionPayload): Promise<any> {
    return this.McpServerService.getMcpServerByName(name);
  }

  @Get()
  getMcpServerList(@Req() req: Request, @CurrentUser() user: SessionPayload): Promise<K8sResponse> {
    // any query params
  ...
  @Delete(":name")
  deleteMcpServer(@Param("name") name: string, @CurrentUser() user: SessionPayload): Promise<any> {
